...
# before tts_exp="{expdir}/tts_${tag}"
if [ -z "${tts_exp}" ]; then
    tts_exp="${expdir}/${tag}"
fi
...
if ! "${skip_data_prep}"; then
    if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
        log "Stage 1: Data preparation for data/${group_num}/${train_set}, data/${group_num}/${valid_set}, etc."
        # [Task dependent] Need to create data.sh for new corpus
        local/data.sh ${group_num} ${local_data_opts}
    ...
    if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
        ...
        for dset in "${train_set}" "${valid_set}" ${test_sets}; do
            ...
            utils/copy_data_dir.sh data/${group_num}/"${dset}" "${data_feats}${_suf}/${dset}"
            rm -f ${data_feats}${_suf}/${dset}/{segments,wav.scp,reco2file_and_channel}
            _opts=
            if [ -e data/"${dset}"/segments ]; then
                _opts+="--segments data/${group_num}/${dset}/segments "
            fi
            # shellcheck disable=SC2086
            scripts/audio/format_wav_scp.sh --nj "${nj}" --cmd "${train_cmd}" \
                --audio-format "${audio_format}" --fs "${fs}" ${_opts} \
                "data/${group_num}/${dset}/wav.scp" "${data_feats}${_suf}/${dset}"
